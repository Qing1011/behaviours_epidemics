...
# rescale the people who have seen the Q8.9 OR Q8.10
# some people have the range [-28, 28]
# some people have the range [-36, 36] most people have this range
# some people have the range [-32, 32]
...
replace_responses <- function(x) {
  if (is.character(x) || is.factor(x)) {
    # Ensure NA values are not replaced
    x[is.na(x)] <- NA

    # Replace specific responses with numerical values
    x[x == "Do not know"] <- 0
...
dat_agency_scores[2:19] <- data.frame(lapply(dat_agency_scores[2:19], replace_responses))

# delete the rows with no complete answers from col 2:15
# Subset the dataframe to include only columns 2 to 15
subset_df <- dat_agency_scores[, 2:15]
...
######################
plot(eigenvalues,
  type = "b", pch = 19, xlab = "Factor", ylab = "Eigenvalue",
  main = "Scree Plot"
)
abline(h = 1, col = "red", lty = 2)
...
plot_scatter <- function(base_name, width = 8, height = 6) {
  ...
  ggplot(point_counts, aes_string(x = col1, y = col2, color = "count", size = "count")) +
    ...
    theme_minimal() +
    labs(
      x = "Availability", y = "Realisticity", color = "Count",
...
coef(my.grm, IRT = T, simplify = T)$items %>%
  round(3) %>%
  as.data.frame() %>%
  paged_table()
